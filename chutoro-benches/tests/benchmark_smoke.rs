...
/// Asserts that `haystack` contains `needle`, reporting the full haystack on
/// failure. Written as a macro rather than a function so panic locations
/// point at the assertion's call site.
...
/// Asserts that `haystack` does not contain `needle`, reporting the full
/// haystack on failure. Written as a macro rather than a function so panic
/// locations point at the assertion's call site.
...
/// Pure assertion over a `--list` discovery output: every entry in
/// `expected_entries` must be present, and `absent_benchmark` (an exact-run
/// marker that discovery must not trigger) must be absent. Kept separate from
/// the fallible [`cargo_bench_output`] query so no `Result`-returning
/// function contains an assertion.
fn assert_discovery_entries(output: &str, expected_entries: &[String], absent_benchmark: &str) {
...
fn benchmark_binaries_cover_discovery_and_exact_smoke_paths() -> TestResult {
    ...
    let neighbour_scoring_list_output = cargo_bench_output("neighbour_scoring", &["--list"])?;
    assert_discovery_entries(
        &neighbour_scoring_list_output,
        &[
