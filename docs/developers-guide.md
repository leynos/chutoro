...
one that no run-unique expression (`github.run_id`, `github.sha`) builds and
that names a per-pull-request expression, and exactly the event-conditioned
...
group, a constant group, the block removed, the trigger renamed to
