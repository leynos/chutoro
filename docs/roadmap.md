...
- [x] 1.6.4. Add Verus proofs for edge harvest primitives:
  ...
  - `CandidateEdge::canonicalize` preserves order and fields.
...
- [x] 2.3.1. Verify and document HNSW neighbour evaluation's packed-index
  adapter path and write-lock-free scoring invariant; keep SoA layout and
  prefetch policy private to dense adapters, with structural follow-ups
  deferred to 2.3.3-2.3.5. (See §6.3 and ADR-003)
...
- [x] 11.1.1. Expose the edge-harvesting HNSW insertion path as a public
  ...
  §12.4)
- [x] 11.1.2. Define `SessionConfig` carrying clustering parameters
...
- [ ] 11.5.1. Add Rustdoc documentation for `ClusteringSession`,
  ...
  seed → append → refresh → read lifecycle. (See `docs/chutoro-design.md`
  §12.3, §12.6)
