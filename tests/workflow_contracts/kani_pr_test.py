...
def test_concurrency_cancels_superseded_runs(workflow: dict[str, object]) -> None:
    """A newer pull-request push cancels the previous run for the same ref.

    Cancellation is conditioned on the event, as `pr_concurrency_test.py`
    requires of every pull-request workflow, so a dispatch is never cancelled.
    """
